// Taken from https://github.com/ShrinathNR/anchor-instruction-sysvar/tree/version-upgrade
...
impl Ed25519InstructionSignatures {
    pub fn unpack(data: &[u8]) -> Result<Self> {
        ...
        let signatures: Vec<Ed25519InstructionSignature> = (0..num_signatures)
            .into_iter()
            .map(|i| {
                ...
                let public_key: Option<Pubkey> =
                    match offsets.public_key_instruction_index == u16::MAX {
                        true => {
                            match Pubkey::try_from(
                                data[offsets.public_key_offset as usize
                                    ..offsets.public_key_offset as usize
                                        + PUBKEY_SERIALIZED_SIZE as usize]
                                    .to_vec(),
                            ) {
                                Ok(it) => Some(it),
                ...
                let signature: Option<[u8; 64]> = match offsets.signature_instruction_index
                    == u16::MAX
                {
                    true => {
                ...
                let message: Option<Vec<u8>> = match offsets.message_instruction_index == u16::MAX {
                    true => Some(
                        data[offsets.message_data_offset as usize
